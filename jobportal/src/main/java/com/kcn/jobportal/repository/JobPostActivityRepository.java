package com.kcn.jobportal.repository;


import com.kcn.jobportal.entity.IRecruiterJobs;
import com.kcn.jobportal.entity.JobPostActivity;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;

import java.time.LocalDate;
import java.util.List;

public interface JobPostActivityRepository extends JpaRepository<JobPostActivity, Integer> {

    @Query(value = """
    SELECT COUNT(s.user_id) as totalCandidates,
           j.job_post_id     as jobPostId,
           j.job_title       as jobTitle,
           l.id              as locationId,
           l.city            as city,
           l.state           as state,
           l.country         as country,
           c.id              as companyId,
           c.name            as name
    FROM job_post_activity j
             INNER JOIN job_location l ON j.job_location_id = l.id
             INNER JOIN job_company c ON j.job_company_id = c.id
             LEFT JOIN job_seeker_apply s ON s.job = j.job_post_id
    WHERE j.posted_by_id = :recruiter
    GROUP BY j.job_post_id, j.job_title, l.id, l.city, l.state, l.country, c.id, c.name
    """, nativeQuery = true)
    List<IRecruiterJobs> getRecruiterJobs(@Param("recruiter") int recruiter);

    @Query(value = """
    SELECT * FROM job_post_activity j
    INNER JOIN job_location l ON j.job_location_id = l.id
    WHERE (CAST(:job AS text) IS NULL OR :job = '' OR j.job_title ILIKE '%' || :job || '%')
    AND (CAST(:location AS text) IS NULL OR :location = '' OR 
        (l.city ILIKE '%' || :location || '%' OR 
         l.country ILIKE '%' || :location || '%' OR 
         l.state ILIKE '%' || :location || '%'))
    AND (COALESCE(ARRAY_LENGTH(:remote, 1), 0) = 0 OR j.remote = ANY(:remote))
    AND (COALESCE(ARRAY_LENGTH(:type, 1), 0) = 0 OR j.job_type = ANY(:type))
""", nativeQuery = true)
    List<JobPostActivity> searchWithoutDate(
            @Param("job") String job,
            @Param("location") String location,
            @Param("remote") String[] remote,
            @Param("type") String[] type);

    @Query(value = """
    SELECT * FROM job_post_activity j
    INNER JOIN job_location l ON j.job_location_id = l.id
    WHERE (CAST(:job AS text) IS NULL OR :job = '' OR j.job_title ILIKE '%' || :job || '%')
    AND (CAST(:location AS text) IS NULL OR :location = '' OR 
        (l.city ILIKE '%' || :location || '%' OR 
         l.country ILIKE '%' || :location || '%' OR 
         l.state ILIKE '%' || :location || '%'))
    AND (COALESCE(ARRAY_LENGTH(:remote, 1), 0) = 0 OR j.remote = ANY(:remote))
    AND (COALESCE(ARRAY_LENGTH(:type, 1), 0) = 0 OR j.job_type = ANY(:type))
    AND
    j.posted_date >= CAST(:dateParam AS timestamp)
    """, nativeQuery = true)
    List<JobPostActivity> search(
            @Param("job") String job,
            @Param("location") String location,
            @Param("remote") String[] remote,
            @Param("type") String[] type,
            @Param("dateParam") LocalDate date);






    }
